//onload function mei saara code wrap kara hai kyuki it mightbe possible ki winfow load naa ho aur code execute hojaye toh ise problem aa skti hai
...
onload = function () {
    ...
    function createData(){
        //sz=size basicaly  to create  random vertices in a graph
        sz = Math.floor(Math.random() * 8) + 3;
        // sz = Math.floor(Math.random());
        console.log(sz) ;
        ...
        for(let i=2;i<=sz;i++){
            ...
            //  isse ek problem aa rhi thi ki jaadatar edges 1 and 2 node se hi connect ho rhi thi toh humne neeche sai kra usko
            // toh humne kya iya min lagadiya agar 3 se kam node hongi toh wo ple wala lelega and usse jada mei wo rndom value genrate krega
        ...
        src = 1;
        dst = sz;
        //uppar wala code bs tree banayga vis.js se ar edges srf bus ki ongi usmei toh planes ki edges hum manually add karenge
    ...
    function solveData(sz) {
        ...
        //  minimum distance from source to destination ,this is only by bus
        let dist1 = dijkstra(graph,sz,src-1);
        //minimum distance from destination to source,only by bus
